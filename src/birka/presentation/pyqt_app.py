...
class DiagramView(QtWidgets.QGraphicsView):
    def __init__(self, diagram: Diagram, parent: QtWidgets.QWidget | None = None) -> None:
    ...
    def _draw_tasks(self, tasks: Iterable[TaskView]) -> None:
        ...
        for task in tasks:
            rect = QtCore.QRectF(task.rect.left, task.rect.top, task.rect.width, task.rect.height)
            ...
            text_item.setTextWidth(task.rect.width - 10)
            text_item.setPos(task.rect.left + 5, task.rect.top + 5)
